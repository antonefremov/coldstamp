import { isSensitiveField, redactValue, redactFreeText } from "../lib/redact";
...
const interactedEls = new WeakSet();
const interactionLog = [];
const networkCharges = [];
const elSelector = (el) => {
...
const pushInteraction = (kind, target) => {
    interactionLog.push({
        t: Date.now(),
        kind,
        target: target ? elSelector(target) : "window",
    });
    if (interactionLog.length > 500)
...
document.addEventListener("focusin", (e) => pushInteraction("focus", e.target), true);
...
function extractChargeFromBody(body) {
    let amount = null;
    let currency = null;
    let recurring = null;
    ...
    try {
        ...
        if (j && typeof j === "object") {
            if (amount == null && typeof j.amount === "number")
                amount = j.amount;
            if (!currency && typeof j.currency === "string")
                currency = j.currency;
...
function looksLikeCheckout() {
    ...
    const ccField = document.querySelector('input[autocomplete="cc-number"], input[name*="card" i], input[id*="card" i][type="text"], input[type="tel"][name*="card" i]');
    if (ccField)
        return true;
    // Known payment SDK loaded?
    const scripts = Array.from(document.scripts).map((s) => s.src.toLowerCase());
    if (scripts.some((s) => /stripe|braintree|adyen|paypal|checkout\.com/.test(s)))
        return true;
    // Subscription keyword near a primary CTA?
    const buttons = Array.from(document.querySelectorAll('button, [role="button"], input[type="submit"]'));
    if (buttons.some((b) => KEYWORDS.test(b.textContent || b.value || "")))
        return true;
    return false;
}
...
function snapshotControls() {
    ...
    inputs.forEach((el) => {
        ...
        const labelText = redactFreeText(labelFor(el).slice(0, 200));
        const checked = type === "checkbox" || type === "radio" ? el.checked : null;
        const defaultChecked = type === "checkbox" || type === "radio" ? el.defaultChecked : null;
        const rawValue = "value" in el ? String(el.value ?? "") : "";
        const value = redactValue(rawValue, sensitivity);
        const defaultValue = redactValue(String(el.defaultValue ?? ""), sensitivity);
        let rect = null;
        try {
