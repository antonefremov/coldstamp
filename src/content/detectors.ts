...
export function isCheckoutLikePage(): boolean {
  const url = (location.pathname + location.search).toLowerCase();
  if (
    /check\s?out|subscribe|signup|sign-up|join|cart|billing|payment|upgrade|plan|order|purchase/.test(
      url
    )
  ) {
    return true;
  ...
  return btns.some((b) =>
    /pay|subscribe|start (free|trial)|place order|complete (order|purchase)|continue to payment|join now|upgrade/i.test(
      text(b) || (b as HTMLInputElement).value || ""
    )
  );
}

// ---- Detector 1: pre-selected recurring opt-in ------------------------------
function detectPreselectedRecurring(): Finding[] {
  const out: Finding[] = [];
  ...
  checked.forEach((input) => {
    ...
    if (recHit) {
      out.push({
        ...
        detail:
          `An option that signs you up for a repeating charge is already selected ` +
      ...
      out.push({
        ...
        detail:
          `An optional extra is already selected for you ("${truncate(label, 90)}"). ` +
...
function detectTrialConversion(): Finding[] {
  const out: Finding[] = [];
  const bodyText = text(document.body);
  if (!containsAny(bodyText, lexicon.trialCta)) return out;
  ...
  for (const { el, price } of priceNodes) {
    ...
    if (convHit && price.cadence) {
      const daysMatch = ctx.match(
        /(\d{1,3})[ -]?(day|week|month)s?\s+(?:free|trial)|(?:free|trial)[^.]{0,20}?(\d{1,3})[ -]?(day|week|month)s?/
      );
      let when = "after the trial ends";
      if (daysMatch) {
        const num = daysMatch[1] || daysMatch[3];
        const unit = daysMatch[2] || daysMatch[4];
        if (num) when = `in ${num} ${unit}${num === "1" ? "" : "s"}`;
      }
      out.push({
        id: "trial-conversion",
        severity: "medium",
        ...
        detail:
          `This "free" offer converts to ${price.raw}/${price.cadence} ${when}. ` +
...
function detectBuriedPrice(): Finding[] {
  const out: Finding[] = [];
  ...
  if (!recurring.length) return out;

  let headline: { el: HTMLElement; fs: number } | null = null;
  for (const n of nodes) {
    const fs = fontSize(n.el);
    if (!headline || fs > headline.fs) headline = { el: n.el, fs };
  }
  ...
  for (const r of recurring) {
    ...
    if ((muchSmaller || lowContrast) && r.el !== headline.el) {
      out.push({
        ...
        detail:
          `The ongoing cost (${r.price.raw}/${r.price.cadence}) is shown much smaller ` +
...
function detectTotalMismatch(): Finding[] {
  const out: Finding[] = [];
  ...
  for (const lab of labels) {
    ...
    if (p && p.amount > totalVal) totalVal = p.amount;
  }
  if (!totalVal) return out;
  ...
  if (sum > 0 && totalVal - sum > Math.max(1, sum * 0.05) && totalVal - sum < totalVal) {
    out.push({
      ...
      detail:
        `Your items total about ${sum.toFixed(2)}, but the order total is ` +
...
function detectConfirmshaming(): Finding[] {
  const out: Finding[] = [];
  const candidates = Array.from(
    document.querySelectorAll<HTMLElement>('a, button, span[role="button"], [role="button"]')
  ).filter(isVisible);
  for (const el of candidates) {
    ...
    if (containsAny(t, lexicon.confirmshame)) {
      out.push({
        ...
        detail:
          `The "no" option is worded to make you feel bad for declining ` +
...
export function runDetectors(): Finding[] {
