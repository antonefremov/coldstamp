...
export function isSensitiveField(opts) {
    ...
    if (ac === "cc-csc")
        return "cvv";
    if (SENSITIVE_AUTOCOMPLETE.has(ac))
        return "card";
    if (opts.type === "email" || ac === "email")
        return "email";
    const hint = `${opts.name} ${opts.id}`;
    if (SENSITIVE_NAME_HINT.test(hint))
        return "card";
    return null;
}
export function redactValue(value, kind) {
    if (!value)
        return value;
...
export function redactBodySnippet(body) {
