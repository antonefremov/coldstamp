...
export function labelFor(input: HTMLElement): string {
...
export function parsePrice(str: string): ParsedPrice | null {
  ...
  let cadence: ParsedPrice["cadence"] = null;
  if (/\/\s?mo\b|per month|a month|monthly|\/month/i.test(str)) cadence = "month";
  else if (/\/\s?yr\b|per year|a year|annually|yearly|\/year/i.test(str)) cadence = "year";
  else if (/\/\s?wk\b|per week|a week|weekly|\/week/i.test(str)) cadence = "week";
  return { amount, cadence, raw: m[0] };
}

export type PriceNode = { el: HTMLElement; price: ParsedPrice; text: string };

export function findPriceNodes(root: Element = document.body): PriceNode[] {
  const out: PriceNode[] = [];
  ...
  while ((n = walker.nextNode())) {
    ...
    if (!el || !n.nodeValue) continue;
    const price = parsePrice(n.nodeValue);
    if (price) out.push({ el, price, text: n.nodeValue.trim() });
...
export function contrastRatio(el: Element): number {
...
export function fontSize(el: Element): number {
